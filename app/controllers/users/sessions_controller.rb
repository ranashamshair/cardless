...
class Users::SessionsController < DeviseController
  ...
  def login_verification
    ...
    if result
      ...
      if failed_attempts_checker == 6
        render status: 404, json: { error: 'exceded limit' }
      else
        failed_attempts_checker == 4 || failed_attempts_checker == 5 ? (render status: 404, json: { error: "Invalid password - You have #{remaining_warning} more attempt" }) : (render status: 404, json: { error: 'You are not Authorized for this action.' })
end
    end
  end
